...
export default function Home() {
  ...
  const refreshLiveData = useCallback(async (holder?: Address) => {
    ...
    const [details, proceeds, purchases] = await monadPublicClient.multicall({
      allowFailure: false,
      contracts: [
        { address, abi: fairTicketAbi, functionName: "ticketDetails", args: [holder] },
        { address, abi: fairTicketAbi, functionName: "sellerProceeds", args: [holder] },
        { address, abi: fairTicketAbi, functionName: "lifetimePurchases", args: [holder] },
      ],
    });
    ...
    const liveTickets = details[0].map((tokenId, index) => {
        const status: TicketStatus = details[1][index]
          ? "used"
          : details[2][index] > 0n
            ? "listed"
            : "ready";
        return {
          id: Number(tokenId),
          status,
          listingPrice: details[2][index] || undefined,
          purchasePrice: details[3][index],
          maxResalePrice: details[4][index],
        };
      });
                  ...
                  {ticket.status === "ready" ? (
                    <VStack gap={3}>
                      ...
                      <HStack gap={3} wrap="wrap">
                        <Button label="List ticket" variant="secondary" isDisabled={isBusy} clickAction={() => listTicket(ticket.id)} />
